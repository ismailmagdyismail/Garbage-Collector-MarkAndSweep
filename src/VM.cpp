...
{
    return AllocateObject(CreateIntObject(p_iValue));
}

VMObject *VM::CreatePair(VMObject *p_pFirst, VMObject *p_pSecond)
{
    return AllocateObject(CreatePairObject(p_pFirst, p_pSecond));
}

VMObject *VM::CreateArray(const std::vector<VMObject *> &p_vecElements)
{
    return AllocateObject(CreateArrayObject(p_vecElements));
}

VMObject *VM::CreateString(const std::string &p_strValue)
{
    return AllocateObject(CreateStringObject(p_strValue));
}

VMObject *VM::AllocateObject(VMObject *p_pVMObject)
{
