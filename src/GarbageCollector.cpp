...
{
    return GetObjectDescriptor(p_pVMObject->m_eObjectType).m_fnDestroy(p_pVMObject);
}
...
unsigned int GarbageCollector::Mark(VMObject *p_pVMObject)
{
    ...
    unsigned int uiReachableElements = 1;
    GetObjectDescriptor(p_pVMObject->m_eObjectType)
        .m_fnTrace(
            p_pVMObject,
            [&](VMObject *p_pChildObject)
            {
                uiReachableElements += Mark(p_pChildObject);
            });
